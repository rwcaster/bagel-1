...
def fit(data):
    ...
    if r_value > 0.95:
        ...
        kcat, err1, km, err2 = [None, None, None, None] 
    else:
...
plates = pandas.read_csv('bagel.csv')
fits = plates.groupby(by='Sample').apply(fit)
#fits = plates.groupby(by='Sample').apply(fit).to_csv("out.csv")
...
fits.plot(kind='bar',logy=True, figsize=(33,3), y='kcat')
plt.xlabel('')
plt.ylabel('kcat')
plt.show()

fits.plot(kind='bar',logy=True, figsize=(33,3), y='km')
plt.xlabel('')
plt.ylabel('km')
plt.show()

fits.plot(kind='bar',logy=True, figsize=(33,3), y='eff')
plt.xlabel('')
plt.ylabel('kcat/km')
plt.show()
